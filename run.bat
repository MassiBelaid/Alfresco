...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f test-share
	call %MVN_EXEC% clean package -pl test-share,test-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill test-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f test-acs
	call %MVN_EXEC% clean package -pl test-integration-tests,test-platform,test-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl test-platform,test-integration-tests,test-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl test-platform,test-integration-tests
EXIT /B 0
:purge
